...
impl <META: ByteSerializable + KnownSize>InodeTable<META> {
    pub fn create(index: BlockPointer, io: &mut IO) -> InodeTable<META> {
        ...
        InodeTable { map: vec![0u8; (inode_count / 8u64) as usize], map_index: index, inode_count, table_index: index + map_blocks, block_count: total_blocks as usize, meta: PhantomData }
    }
    ...
    pub fn read(io: &IO, index: BlockPointer, inode_count: u64) -> InodeTable<META> {
        ...
        InodeTable { map, map_index: index, inode_count, table_index: index + map_blocks, block_count: total_blocks as usize, meta: PhantomData }
    }
    ...
    pub fn write_inode(&mut self, io: &mut IO, inode: &mut Inode<META>) {
        match inode.id {
            ...
            Some(index) => {
                ...
                let mut block = io.read_block(inode_block);
                block[offset..offset + Inode::<META>::size_on_disk()].copy_from_slice(inode.to_bytes().as_slice());
    ...
    fn inode_offset(index: InodePointer, block_size: usize) -> usize {
        (index % (block_size / Inode::<META>::size_on_disk()) as u64) as usize * Inode::<META>::size_on_disk()
    }
